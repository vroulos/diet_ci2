<!DOCTYPE html>
<html>
<head>

	<link href="<?= base_url('assets/css/bootstrap.min.css') ?>" rel="stylesheet">
	<link href="<?= base_url('assets/css/style.css') ?>" rel="stylesheet">
	<!-- google fonts -->
	<link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet"> 

	<title>connect to eat better</title>

	<style type="text/css">
		.container {
			display: flex; /* or inline-flex */
			justify-content: space-around;

		}

		img{
			height: 300px;
			width: 300px;
		}
		h1{


			font-family: 'Roboto', sans-serif;


		}


	</style>

</head>
<body style="background-color: #fafafa">
	<h1 class="text-left">Καλως όρισες στην εφαρμογή </h1>



	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		<ul class="nav navbar-nav navbar-right">

			<li><a href="<?= base_url('user/login') ?>">Είσοδος Χρήστη</a></li>
			<li><a href="<?= base_url('dietitians/logind') ?>">Είσοδος Διαιτολόγου</a></li>
		</ul>
	</div>

	<div class="container-fluid" >
		<img src="<?= base_url('assets/images/nutricion.jpg')?>">
		<img src="<?= base_url('assets/images/consistency.jpg')?>">
		<img src="<?= base_url('assets/images/exercise.jpg')?>">

	</div>
	<div>
		<p class="font-weight-bold">Εξατομικευμένη διατροφή για όλους</p>
		<p class="text-left">Το σύστημα είναι ένα λογισμικό διατροφής για διαιτολόγους και επαγγελματίες της διατροφής που παρέχει την τεχνολογία για να δίνουν εύκολα εξατομικευμένη διατροφή, να προσαρμόζουν τα προγράμματα διατροφής σε πραγματικό χρόνο και να παραμένουν συνδεδεμένοι με κάθε έναν από τους πελάτες τους.</p>
		<div id="grey-div-mywebapp" >
			<p class="font-weight-bold text-center" style="font-size: 30px">Γιατί να επιλέξετε το nutricio</p>
			<div>
				<dl class="row text-left">
					<div class="col-sm">
						<dt >Καλύτερη διαχείριση χρόνου</dt>
						<dd>Δημιουργήστε εξατομικευμένα διατροφικά προγράμματα γρηγορότερα από ό, τι χρειάζεται για να φτιάξετε ένα σάντουιτς, επιτρέποντάς σας να βοηθήσετε περισσότερους πελάτες.</dd>

					</div >
					<div class="col-sm">
						<dt >Αλληλεπίδραση σε πραγματικό χρόνο</dt>
						<dd >Μείνετε συνδεδεμένοι με τους ασθενείς σας και είστε εκεί για κάθε βήμα προς τους στόχους τους.</dd>

					</div>
					<div class="col-sm">
						<dt >Πιο ευτυχισμένοι πελάτες</dt>
						<dd >Εστιάστε στους πελάτες σας και κρατήστε τους να παρακινούνται για την επίτευξη των στόχων τους.</dd>
					</div>
				</dl>	
			</div>

		</div>
		<p class="font-weight-bold">Εξατομικευμένα προγράμματα διατροφής</p>
		<p class="text-left">Ο σχεδιασμός εξατομικευμένων προγραμμάτων για τους πελάτες είναι εύκολος. Μπορείτε να δημιουργήσετε καθημερινά προγράμματα γεύματος και να τα προσαρμόσετε σε πραγματικό χρόνο με βάση τις ανάγκες των πελατών.</p>
		<p class="font-weight-bold">Αξιολογήσεις(feedback) πελατών</p>
		<p class="text-left">Ζητήστε από τους πελάτες σας να σας δώσουν αξιολόγηση και να σας ενημερώσουν όποτε μπαίνουν στον πειρασμό να τρώνε έξω από το πρόγραμμα γευμάτων, ώστε να μπορείτε να προτείνετε νέες υγιεινές επιλογές που τους ταιριάζουν.</p>
		<p class="font-weight-bold">Αξιοποίηση αποτελέσματων</p>
		<p class="text-left">Δείτε ανά πάσα στιγμή ποια δίαιτα είναι πιο επιτυχημένη με βάση τα σχόλια των πελατών σας.</p>
		<p class="font-weight-bold">Responsive σχεδιασμός</p>
		<p class="text-left">Η εφαρμογή είναι προσβάσιμη ανά πάσα στιγμή, οπουδήποτε, από οποιαδήποτε συσκευή.</p>
		<p class="font-weight-bold">Μείνετε πλήρως συνδεδεμένοι με τους ασθενείς σας μέσω του λογισμικού διατροφής</p>
		<p class="text-left">Εξαρτάται από εσάς πότε και πώς επικοινωνείτε, κάτι που σας δίνει τόσο την ελευθερία να σχεδιάσετε το πρόγραμμά σας όσο και την ευκαιρία να εξασφαλίσετε εξαιρετικά αποτελέσματα, όταν είστε εκεί για τους ασθενείς όποτε χρειάζονται και δίνοντάς τους κίνητρα.</p>
	</div>
</div>
</body>
</html>